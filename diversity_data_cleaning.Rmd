...
---
## IMPORTANT NOTE: to have fully cleaned data, need to determine exclusion criteria
...
adult_data <- read_csv("diversity_adult_data.csv")
child_data <- read_csv("diversity_child_data.csv")

# Two child participants have same ID (two 1795M). Changning second 1795M to 1795Ma
child_data$id <- ifelse(child_data$id == "1795M" & child_data$calibration == .65, "1795Ma", child_data$id)
...
# filtering out participants who answered incorrectly on Audiocheck2, Audiocheck3, warmups, and lunch and laundry2
check <- subset(
  check,
  correct == "0" &
    (stim1 == "AudioCheck2" |
...
data_demographics <- data %>% filter(type == "form") %>% select(id, head, response, age)

# keeping only test trials in main dataset since we no longer need attention check trials
data %<>% filter(type == "test" & !stim1 %in% c("AudioCheck", "AudioCheck2", "AudioCheck3", "warmup", "warmup2", "lunch1", "lunch2", "laundry1", "laundry2", "download-4", "laundry11", "laundry21final")) %>% select(id, stim1, response, age)
